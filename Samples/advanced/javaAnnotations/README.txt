...
As of Micro Focus Fortify 4.30 release, two versions of Java Fortify Annotations library are provided.  One version contains annotations with retention policy set to SOURCE (FortifyAnnotations-SOURCE.jar).  This means that Fortify annotations will not be propagated to the bytecode after the code that uses them is compiled.  The other version contains the same annotations with retention policy set to CLASS (FortifyAnnotations-CLASS.jar).  When using this version, Fortify annotations will be propagated to the bytecode during compilation.

If you use Fortify products to analyze bytecode of your applications (e.g.  Fortify on Demand assessments), then the version with annotation retention policy set to CLASS should be used.  If you use Fortify products to analyze source code of your applications, you can use either version of the library, however we strongly suggest that you use the one with retention policy set to SOURCE.

Leaving Fortify annotations in production code is a security risk because they can leak information about potential security problems in the code.  We recommend that you use annotations with retention policy set to CLASS only for Fortify builds and assessments, and never use them for production builds of your software.

This document outlines the annotations available, along with the sample provided with the Java Annotations.  This document discusses the sample application, problems that may arise from this sample application, and how to fix these problems using the Fortify Java Annotations.

Currently, there are a couple limitations. Each annotation can only specify one input and/or one output. Also, only one annotation of each type may be applied to the same target.

We provide three main types of annotations:

   1. Dataflow Annotations
   2. Field and Variable Annotations
   3. Other Annotations
...
Dataflow annotations, like Dataflow rules, come in four types: Source, Sink, Passthrough, and Validate. All are applied to methods and specify the inputs and/or outputs by parameter name or the strings "this" and "return".

Additionally, the Dataflow Source and Sink annotations can now be applied to the function arguments. 
...
Assigns taint to an output of the target method. Acceptable values for the annotation parameter are "this", "return", or one of the function parameter names.

@FortifyDatabaseSource("this")
void executeQuery() {
...
}
...
Assigns taint to an argument of the target method.
...
In addition to specific source annotations, we provide a generic "untrusted" taint source: FortifySource.

The complete list of source annotations:
...
Transfers any taint from an input to an output of the target method. May also assign or remove taint from the output, in the case of FortifyNumberPassthrough and FortifyNotNumberPassthrough. Acceptable values for the in annotation parameter are "this" or one of the function parameter names. Acceptable values for the out annotation parameter are "this", "return", or one of the function parameter names.
...
FortifyNumber is used to indicate that the data is purely numeric. Numeric data cannot cause certain types of issues, such as cross-site scripting, regardless of source. Using the FortifyNumber passthrough can reduce false positives of this type. If a program decomposes some character data into a numeric type (int, int[], etc) then FortifyNotNumber can be used to specify that, although the data is numeric in form, at the time it is returned or operated on it will be converted back into character data.

The complete list of passthrough annotations:
...
Removes taint from an output of the target method. Acceptable values for the annotation parameter are "this", "return", or one of the function parameter names.
...
The complete list of sink annotations:
...
Reports an issue when taint of the appropriate type reaches an input of the target method. Acceptable values for the annotation parameter are "this" or one of the function parameter names.
...
Annotation can also be applied to the function argument or the return parameter. In the example below, an issue is reported when taint reaches argument 'a'.
...
These annotations can be applied to fields and (in most cases) variables.

Password and Private annotations
--------------------------------

These annotations are used to indicate whether the target field or variable is a password or private data.
...
String x will now be identified as a password and checked for privacy violations and hardcoded passwords. String pass will not be identified as a password. Without the annotation, it would be, possibly causing false positives. The FortifyPrivate and FortifyNotPrivate annotations work similarly, only they do not cause hardcoded password issues.


NonNegative and NonZero annotations
-----------------------------------

These annotations are used to indicate disallowed values for the target field or variable.

@FortifyNonNegative int index;
@FortifyNonZero double divisor;

An issue will be reported if a negative value is assigned to index or zero is assigned to divisor.
...
The target method will be added to the list of functions that require a check of the return values.
...
Any use of the target function, field or class will be reported. Acceptable values for the annotation parameter are "CRITICAL", "HIGH", "MEDIUM", or "LOW" (indicating the Fortify Priority Order values of where to place the issues). For those using an older version of SCA, or an older project template, the values "HOT", "MEDIUM", and "INFO" can also be used to indicate the relevant folders.
